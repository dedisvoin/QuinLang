...
from src.ml_parser.expressions import (
    IntExpr, FloatExpr, StringExpr, UnaryExpr, BinaryExpr, VarExpr, 
    ArrayExpr, FunctExpr, BoolExpr, CondExpr, ArrayGetExpr, DiapozonExpr, NoneExpr, LambdaExpr
)
...
class Parser:
    ...
    def primary(self):
        ...
        # Diaposon
        if self.match(TokenTypes.SNAKE):
            if self.match(TokenTypes.LEFT_RECT_BRACK):
            ...
            if self.match(TokenTypes.DOUBLE_DOT):
                if self.match(TokenTypes.LEFT_BRACK):
                    while not self.match(TokenTypes.RIGHT_BRACK):
                        ...
                        self.match(TokenTypes.COMMA)
                return lambda_expr.eval(evaluate_args)
            else:
                return lambda_expr
